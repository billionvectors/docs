...
<Tip>
If you are using Docker, this step is unnecessary as Docker will automatically copy the `.env.local` file to `.env` when running the container.
</Tip>
...
- `ATV_ENABLE_SWAGGER_UI`: Enable or disable Swagger UI (true: enable, false: disable).
- `ATV_RAFT_HEARTBEAT_INTERVAL`: Set the Raft heartbeat interval in milliseconds. (min 0 ~ max 299)
- `ATV_RAFT_ELECTION_TIMEOUT`: Set the Raft election timeout in milliseconds. (min 0 ~ max 299)
...
- `--enable_swagger_ui`: Enable or disable Swagger UI (true: enable, false: disable).
- `--raft_heartbeat_interval`: Set the Raft heartbeat interval in milliseconds.
- `--raft_election_timeout`: Set the Raft election timeout in milliseconds.
...
<Tip>
These default values may change depending on the version of the software. For the latest default settings, please refer to the [config.rs file](https://github.com/billionvectors/asimplevectors/blob/main/src/config.rs).
</Tip>

## Conclusion

The configuration system in asimplevectors allows for flexible and dynamic management of environment variables and command-line arguments. This enables you to easily adjust settings for different environments or during runtime, providing both versatility and control over the system’s behavior.